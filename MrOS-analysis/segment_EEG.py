...
def segment_EEG(EEG, sleep_stages, window_time, step_time, Fs, newFs=200, notch_freq=None, bandpass_freq=None, start_end_remove_window_num=0, amplitude_thres=500, n_jobs=1, to_remove_mean=False):#
    """Segment EEG signals.
    ...
    lens = [len(EEG_segs), len(sleep_stages),len(start_ids), len(epoch_statuss), len(specs)]
    assert len(set(lens))==1

    return EEG_segs, sleep_stages, start_ids, np.array(epoch_statuss), specs, freq, qs
